    ...
    if (isoBody) {
      ...
      if (el.isotopes && el.isotopes.length > 0) {
        el.isotopes.forEach(iso => {
          ...
          const abundStr = iso.abundance_str || (iso.abundance_percent !== null && iso.abundance_percent !== undefined
            ? `${iso.abundance_percent.toFixed(2)}%`
            : (typeof iso.abundance === 'string' ? iso.abundance : (iso.is_synthetic ? 'Synthetic' : 'Trace')));
          const halfLifeStr = iso.is_stable ? '<span class="text-emerald-400 font-semibold">Stable</span>' : (iso.half_life || 'Radioactive');
          const decayStr = iso.decay_mode && iso.decay_mode !== 'None' ? ` <span class="text-[10px] text-amber-400 font-mono">(${iso.decay_mode})</span>` : '';

          tr.innerHTML = `
            <td class="py-1 font-bold text-white font-mono">${iso.nuclide || `${iso.mass_number || ''}${el.symbol}`}</td>
            <td class="py-1 text-slate-300 font-mono">${massStr}</td>
            <td class="py-1 text-cyan-300 font-mono">${abundStr}</td>
            <td class="py-1 font-mono">${halfLifeStr}${decayStr}</td>
          `;
